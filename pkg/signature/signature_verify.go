...
func (s *signature) Verify(authorization, date string, path string, method string, params url.Values) (ok bool, err error) {
	...
	msg := fmt.Sprintf("Signature verification failed, path: %s, method: %s, params: %s, date: %s", path, methodName, sortParamsEncode, date)
	msg += fmt.Sprintf("signature verify:%s %s", authorization, digest)

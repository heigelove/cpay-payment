...
import (
	...
	"github.com/heigelove/cpay-payment/assets"
	"github.com/heigelove/cpay-payment/configs"
	_ "github.com/heigelove/cpay-payment/docs"
	"github.com/heigelove/cpay-payment/internal/code"
	"github.com/heigelove/cpay-payment/internal/proposal"
	"github.com/heigelove/cpay-payment/pkg/browser"
...
const _UI = "Server started successfully"
...
// WithEnableSafeCheck 启用安全检查
func WithEnableSafeCheck() Option {
	return func(opt *option) {
		opt.enableSafeCheck = true
	}
}
...
func New(logger *zap.Logger, options ...Option) (Mux, error) {
	...
	// 安全检查
	if opt.enableSafeCheck {
		mux.engine.Use(safecheck.SafeCheckWithConfig(safecheck.SafeCheckConfig{
	...
	// 添加静态资源和HTML模板的缓存控制
	mux.engine.Use(func(c *gin.Context) {
		// 静态资源缓存控制
		if strings.HasPrefix(c.Request.URL.Path, "/assets/") {
			c.Header("Cache-Control", "public, max-age=31536000") // 静态资源缓存一年
			c.Header("Expires", time.Now().AddDate(1, 0, 0).Format(http.TimeFormat))
		}

		// HTML页面缓存控制
		if strings.HasSuffix(c.Request.URL.Path, ".html") || c.GetHeader("Accept") == "text/html" {
			c.Header("Cache-Control", "no-cache, must-revalidate") // HTML不缓存或需要验证
	...
	// 设置静态资源和HTML模板
	mux.engine.StaticFS("/assets", http.FS(assets.Bootstrap)) // 静态资源目录

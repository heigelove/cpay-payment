...
// GenerateTempToken generates a secure random token string using the admin ID as a prefix.
func GenerateTempToken(adminID int32) string {
	b := make([]byte, 16)
	_, err := rand.Read(b)
	if err != nil {
		return strconv.FormatInt(int64(adminID), 10) + "_fallbacktoken"
	}
	return strconv.FormatInt(int64(adminID), 10) + "_" + hex.EncodeToString(b)
}
